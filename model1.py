"""Implementation of a simple multilayer neural network from scratch as part
of Assignment 1 of CISC 874 Neural and Cognitive Computing.
...
class Model:
    """Class for training and testing the one-layer neural network."""
    ...
    def __init__(self, x_train, y_train, x_test, y_test, x_val, y_val, batch_size=1):
        ...
        # Initialize network
        self.network = OneLayerNN(input_size, n_hidden, self.num_classes)
    ...
    def train(self, epochs, early_stop=True, min_delta=0, patience=3):
        ...
        for epoch in tqdm.tqdm(range(epochs)):
            ...
            with tqdm.tqdm(range(batches_per_epoch), position=0, leave=True) as pbar:
                for batch_idx in pbar:
                    ...
                    cm = get_confusion_matrix(pred, y_batch)
            ...
            val_cm = get_confusion_matrix(val_pred, self.y_val)
                ...
                if previous_val_loss:
                    if val_loss - previous_val_loss >= min_delta:
                        ...
                        if epochs_without_improvement == patience:
                            ...
                            save_confusion_matrix(val_cm, self.save_path, self.num_classes, "train")
                            break
    ...
    def test(self, model_path=None):
        ...
        cm = get_confusion_matrix(pred, self.y_test)
        precision = self.get_precision(cm)
        recall = self.get_recall(cm)
        print(f"Results of {self.save_path} on test data:\n"
              ...
              f"\tRecall: {recall:.4f}")
        save_confusion_matrix(cm, self.save_path, self.num_classes, "test")
...
if __name__ == '__main__':
    ...
    model.train(1000, early_stop=True)
    model.test()
